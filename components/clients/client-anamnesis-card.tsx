...
type AnamnesisHistoryRow = {
  ...
  socialPhotoConsent?: boolean | null;
...
type AnamnesisApiResponse = {
  enabled: boolean;
  template: AnamnesisTemplate | null;
  history: AnamnesisHistoryRow[];
  active: AnamnesisHistoryRow | null;
};
...
export function ClientAnamnesisCard({ clientId }: { clientId: string }) {
  const [data, setData] = useState<AnamnesisApiResponse | null>(null);
  ...
  const [hasMicrochip, setHasMicrochip] = useState<"" | "yes" | "no">("");
  const [socialPhotoConsent, setSocialPhotoConsent] = useState<"" | "yes" | "no">("");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/clients/${clientId}/anamnesis`);
      const json = await res.json();
      if (!res.ok) {
        alert(json.error || "Errore caricamento modulo anamnesi");
        return;
      }
      setData(json);
    } finally {
  ...
  const hasTemplate = useMemo(() => Boolean(data?.template), [data]);
  ...
  async function saveAnamnesis() {
    ...
    if (!hasTemplate) {
      alert("Template anamnesi non disponibile");
      return;
    }
    ...
    if (!ownerFullName.trim() || !ownerPhone.trim() || !petName.trim()) {
      alert("Compila proprietario, telefono e nome animale");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/clients/${clientId}/anamnesis`, {
        ...
        body: JSON.stringify({
          ...
          hasMicrochip: hasMicrochip === "" ? null : hasMicrochip === "yes",
          socialPhotoConsent: socialPhotoConsent === "" ? null : socialPhotoConsent === "yes",
      ...
      const json = await res.json();
      if (!res.ok) {
        alert(json.error || "Errore salvataggio anamnesi");
        return;
      }
      setShowModal(false);
      resetForm();
  ...
  async function revoke(recordId: string) {
    ...
    const res = await fetch(`/api/clients/${clientId}/anamnesis/revoke`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const json = await res.json();
    if (!res.ok) {
      alert(json.error || "Errore revoca anamnesi");
      return;
    }
    await load();
  }
  ...
  if (loading) {
    return (
      <Card>
        <h2 className="text-lg font-semibold">Anamnesi prima volta</h2>
        <p className="mt-2 text-sm text-zinc-600">Caricamento modulo anamnesi...</p>
      </Card>
    );
  }
      ...
      {data.active ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
          ...
          <p className="mt-1 text-xs">
            Animale: {data.active.petName} {data.active.petBreed ? `(${data.active.petBreed})` : ""} - Microchip: {yesNoUnknown(data.active.hasMicrochip)}
          </p>
          <div className="mt-2">
        ...
        <p className="text-sm text-zinc-600">Nessun modulo anamnesi attivo per questo cliente.</p>
      )}
        ...
        >
          <Card className="mx-auto flex h-full w-full max-w-none flex-col space-y-4 rounded-none p-4 md:h-[calc(100vh-2rem)] md:rounded-2xl md:p-6">
            ...
            <div className="grid gap-2 md:grid-cols-2 overflow-y-auto pr-1">
              ...
              <select className="h-10 rounded-md border border-zinc-300 px-3 text-sm" value={petType} onChange={(event) => setPetType(event.target.value)}>
                <option value="">Tipo animale (opzionale)</option>
              ...
              <select
                ...
                value={hasMicrochip}
                onChange={(event) => setHasMicrochip(event.target.value as "" | "yes" | "no")}
              >
                <option value="">Microchip: non dichiarato</option>
                <option value="yes">Microchip: Si</option>
                <option value="no">Microchip: No</option>
              </select>
              <select
                className="h-10 rounded-md border border-zinc-300 px-3 text-sm"
                value={socialPhotoConsent}
                onChange={(event) => setSocialPhotoConsent(event.target.value as "" | "yes" | "no")}
              >
                <option value="">Consenso foto social: non dichiarato</option>
                <option value="yes">Consenso foto social: Si</option>
                <option value="no">Consenso foto social: No</option>
              </select>
            ...
            <label className="flex items-start gap-2 text-sm">
              <input type="checkbox" checked={isSeniorDeclared} onChange={(event) => setIsSeniorDeclared(event.target.checked)} className="mt-1" />
              Autocertificazione cane/gatto anziano
            </label>

            <Textarea
              placeholder="Malattie da dichiarare (epilessia, otiti, allergie, ecc.)"
              value={diseasesDeclared}
              onChange={(event) => setDiseasesDeclared(event.target.value)}
            />
