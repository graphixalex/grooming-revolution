...
const DEFAULT_MATTING_TEMPLATE: DefaultTemplate = {
  title: "Modulo in caso di nodi",
  legalText: `Modulo consenso in caso di nodi e feltri
...
Confermo di aver letto e compreso integralmente il presente modulo e acconsento o nego i trattamenti indicati (snodatura e/o tosatura sotto i nodi).`,
};

export async function ensureDefaultMattingConsentTemplate(salonId: string) {
  const existing = await prisma.mattingConsentTemplate.findFirst({
  ...
  await prisma.mattingConsentTemplate.create({
    data: {
      salonId,
      title: DEFAULT_MATTING_TEMPLATE.title,
      legalText: DEFAULT_MATTING_TEMPLATE.legalText,
      version: 1,
      isActive: true,
    },
  });
...
export async function getActiveMattingConsentTemplate(salonId: string) {
  await ensureDefaultMattingConsentTemplate(salonId);

  return prisma.mattingConsentTemplate.findFirst({
    where: { salonId, isActive: true },
